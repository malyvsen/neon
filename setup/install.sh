...
pamac install --no-confirm base-devel eza bat firefox telegram-desktop zsh zsh-autosuggestions starship gnome-shell-extension-gsconnect go docker docker-buildx docker-compose github-cli ttf-bitstream-vera &&
echo "Installed GSConnect for clipboard sharing. Enable the GNOME extension and pair manually." &&
sudo systemctl enable docker.service &&
sudo usermod -aG docker $USER &&

pamac build --no-confirm onlyoffice-bin spotify visual-studio-code-bin ttf-twemoji-color &&
