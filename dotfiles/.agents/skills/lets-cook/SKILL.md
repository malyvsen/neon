...
2. Come up with a rough sketch of a recipe and place it in a canvas. Try to use only the ingredients that the user has provided, but prefer to make a sensible recipe than to be limited to these.
   - Copy [example.canvas.tsx](example.canvas.tsx) to the workspace canvases directory and replace only the recipe constants at the top.
