---
name: lets-cook
description: Comes up with a recipe from ingredients the user has, as an interactive HTML page with a checklist, EU nutrition table, and cooking timeline. Use when the user wants to cook or bake from what is in the kitchen, fridge, or pantry, or asks for a recipe from available ingredients.
...
2. Come up with a rough sketch of a recipe as an HTML page. Try to use only the ingredients that the user has provided, but prefer to make a sensible recipe than to be limited to these.
   - Copy [example.html](example.html) to the workspace's artifact directory and replace only the recipe constants at the start of the script. Open the file in a browser for the user to follow along.
   - `LANGUAGE` is `"en"` or `"pl"` — match the user's input language.
   - Keep nutrient `id`s from the example; only change `value` and `intake`.
